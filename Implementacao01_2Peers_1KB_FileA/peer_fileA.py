...
if __name__ == "__main__":
    ...
    if mode == "seeder":
        # 1. Criação do Arquivo Pequeno (File A) com exatamente 10 KB (10 * 1024 bytes)
        with open("file_A_10KB.txt", "wb") as f:
            f.write(os.urandom(10 * 1024)) 

        # 2. Configuração Padrão: 2 Peers (Ouvindo na 5000, vizinho na 5001), Bloco de 1024 bytes
        peer1 = Peer('127.0.0.1', 5000, [('127.0.0.1', 5001)], block_size=1024)
        peer1.fragment_file("file_A_10KB.txt")
        peer1.run()
        ...
        print("Uso correto: py peer_base.py [seeder|leecher]")