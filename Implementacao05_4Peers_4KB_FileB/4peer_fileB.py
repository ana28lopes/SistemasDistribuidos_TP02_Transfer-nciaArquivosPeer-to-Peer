...
class Peer:
    def __init__(self, host, port, neighbors_list, block_size=4096):
        self.host = host
        self.port = port
        self.neighbors = neighbors_list # Configuração estática de vizinhos em cascata
        ...
        try:
            ...
            if request['action'] == 'REQUEST_META':
                ...
                if block_idx in self.blocks_owned:
                    ...
                    # --- LÓGICA DE LOG RESUMIDO NO SERVIDOR ---
                    if block_idx == 0:
                    ...
                    # A LINHA ABAIXO FOI COMENTADA PARA SILENCIAR O CAOS DE REQUISIÇÕES
            ...
            if i not in self.blocks_owned:
                try:
                    ...
                    if resp['status'] == 'OK':
                        ...
                        # --- LÓGICA DE LOG RESUMIDO NO CLIENTE ---
                        if i == 0:
    ...
    # ==========================================
    # INICIALIZAÇÃO DA TOPOLOGIA EM CASCATA (4 PEERS)
    # ==========================================
    def run(self):
...
if __name__ == "__main__":
    ...
    if mode == "peer1":
        # 1. Criação do Arquivo Médio (File B) variação: 5 MB (5 * 1024 * 1024 bytes)
        with open("file_B_5MB.txt", "wb") as f:
            f.write(os.urandom(5 * 1024 * 1024)) 

        # Peer 1 (Seeder): Porta 5000. Fornece o arquivo para a rede.
        peer1 = Peer('127.0.0.1', 5000, [('127.0.0.1', 5001)], block_size=4096)
        peer1.fragment_file("file_B_5MB.txt")
        peer1.run()
        ...
        # Peer 3: Porta 5002. Pede do Peer 2 e fornece ao Peer 4.
        ...
        print("Uso correto: py peer_4nodes_5MB_limpo.py [peer1|peer2|peer3|peer4]")